...
## Modeling Choices
- **Latent Dynamics**: Nonlinear transition `x_{t+1} = f(x_t, u_t) + \epsilon_t` when controls are present, and `x_{t+1} = f(x_t) + \epsilon_t` otherwise. We adopt the sparse inducing-point variational family from Frigola et al. (2014), i.e. augment with inducing variables `u = f(Z)` and minimize the KL between `q(u)q(x_{0:T})` and the true posterior.
...
2. `TimeseriesWindowDataset` is responsible for windowing and padding, with different window lengths available for training/evaluation; optional control inputs are windowed alongside observations and latents.
3. Instantiate `SparseVariationalGPSSM` and build a `Trace_ELBO` trainer through `SVITrainer`.
4. The training loop triggers `evaluate_model` every `eval_every` steps, reporting RMSE, reconstruction NLL, predictive log-likelihood, and latent RMSE where available; after termination, it re-evaluates on val/test.
5. `rollout_forecast` Monte Carlo–propagates the GP transition (including process noise and posterior samples of inducing targets) and adds observation noise to return mean/std (or samples) over the forecast horizon; a moment-matching fallback is kept for affine observation models.
...
## Extension Hooks
- Controlled datasets already thread `controls` through the dataset/dataloader/model stack; richer control parametrizations can be added by replacing the simple concatenation used in the GP transition input.
